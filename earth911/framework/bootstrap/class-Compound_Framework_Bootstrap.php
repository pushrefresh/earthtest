...
if ( ! class_exists( 'Compound_Framework_Bootstrap' ) ) {

	/**
	* The Bootstrap Framework module
	*/
	class Compound_Framework_Bootstrap extends Compound_Framework_Core {
		...
		public function __construct() {
			...
			if ( ! defined( 'COMPOUND_FRAMEWORK_PATH' ) ) {
				define( 'COMPOUND_FRAMEWORK_PATH', dirname( __FILE__ ) );
			}

			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {

				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Options.php' );         // Redux Options
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Advanced.php' );        // Advanced
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Background.php' );      // Background
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Branding.php' );        // Branding
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Blog.php' );            // Blog
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Breadcrumbs.php' );     // Breadcrumbs
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Header.php' );          // Header
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Typography.php' );      // Typography
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Footer.php' );          // Footer
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Social.php' );          // Social
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Layout.php' );          // layout
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Jumbotron.php' );       // Jumbotron
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Menus.php' );           // Menus
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Nav_Walker.php' );      // NavWalker
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Nav_Menu_Widget.php' ); // NavMenus
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/class-Compound_Navlist_Walker.php' );  // NavLists

				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/widgets.php' );                         // Widgets
				include_once( COMPOUND_FRAMEWORK_PATH . '/includes/gallery.php' );                         // Custom [gallery]

				// instantiate the classes
				global $wpscom_layout;
				$wpscom_layout      = new Compound_Layout();

				global $wpscom_background;
				$wpscom_background  = new Compound_Background();

				global $wpscom_advanced;
				$wpscom_advanced    = new Compound_Advanced();

				global $wpscom_branding;
				$wpscom_branding    = new Compound_Branding();

				global $wpscom_blog;
				$wpscom_blog        = new Compound_Blog();

				global $wpscom_footer;
				$wpscom_footer      = new Compound_Footer();

				global $wpscom_headers;
				$wpscom_headers     = new Compound_Header();

				global $wpscom_jumbotron;
				$wpscom_jumbotron   = new Compound_Jumbotron();

				global $wpscom_menus;
				$wpscom_menus       = new Compound_Menus();

				global $wpscom_typography;
				$wpscom_typography  = new Compound_Typography();

				global $wpscom_breadcrumbs;
				$wpscom_breadcrumbs = new Compound_Breadcrumbs();

				global $wpscom_social;
				$wpscom_social      = new Compound_Social();

				add_filter( 'wpscom_compiler', array( $this, 'styles_filter' ) );

				if ( isset( $wpscom_settings['navbar_social'] ) && $wpscom_settings['navbar_social'] == 1 ) {
					if ( $wpscom_settings['navbar_social_style'] == 1 ) {
						add_action( 'wpscom_inside_nav_end', array( $this, 'navbar_social_bar' ) );
					} else {
						add_action( 'wpscom_inside_nav_end', array( $this, 'navbar_social_links' ) );
					}
				}

				if ( isset( $wpscom_settings['retina_toggle'] ) && $wpscom_settings['retina_toggle'] ) {
			...
			if ( ! class_exists( 'WooCommerce' ) && ! function_exists( 'wpscom_woo_include_files' ) ) {
				add_action( 'wpscom_pre_wrap', array( $this, 'breadcrumbs' ), 99 );
			}
		...
		function navlist_widget_init() {
			...
			register_widget( 'Compound_Nav_Menu_Widget' );
		}
		...
		function nav_menu_css_class( $classes, $item ) {
			...
			$classes[] = 'menu-' . wpscom_transliterate( $slug );
		 ...
		 * Use Compound_Nav_Walker() by default
		 */
		function nav_menu_args( $args = '' ) {
			...
			if ( ! $args['walker'] ) {
				$nav_menu_args['walker'] = new Compound_Nav_Walker();
			}

			if ( ! $args['fallback_cb'] ) {
				$nav_menu_args['fallback_cb'] = 'Compound_Nav_Walker::fallback';
			}
		...
		function breadcrumbs() {
			global $wpscom_settings, $wpscom_breadcrumbs;

			if ( is_front_page() || ( isset( $wpscom_settings['breadcrumbs'] ) && $wpscom_settings['breadcrumbs'] == 0 ) ) {
				return;
			}

			if ( isset( $wpscom_settings['site_style'] ) && $wpscom_settings['site_style'] != 'fluid' ) {
				$class = 'container';
			} else {
				$class = 'fluid';
			}

			if ( class_exists( 'Compound_Breadcrumbs' ) ) {
				echo '<div class="breadTrail ' . $class . '">';
				echo $wpscom_breadcrumbs->breadcrumb( false );
				echo '</div>';
			}
		}
		...
		public function make_dropdown_button( $color = 'primary', $size = 'medium', $type = null, $extra = null, $label = '', $content = '' ) {
			...
			$return = '<div class="btn-group">';
				$return .= '<button type="button" class="' . $wpscom_framework->button_classes( $color, $size, $type, 'dropdown-toggle' ) . '" data-toggle="dropdown">';
		...
		public function styles() {
			 ...
			 * BACKGROUND
			 */
			if ( isset( $wpscom_settings['body_bg']['background-color'] ) && ! empty( $wpscom_settings['body_bg']['background-color'] ) ) {
				$bg  = $wpscom_settings['body_bg']['background-color'];
			} else {
				$bg  = '#ffffff';
			}
			$body_bg = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $bg ) );

			// Calculate the gray shadows based on the body background.
			// We basically create 2 "presets": light and dark.
			if ( Compound_Color::get_brightness( $body_bg ) > 80 ) {
			...
			$bg_brightness = Compound_Color::get_brightness( $body_bg );
			...
			// Calculate grays
			if ( isset( $wpscom_settings['body_bg'] ) && ! empty( $wpscom_settings['body_bg'] ) ) {
			 ...
			 * LAYOUT
			 */
			if ( isset( $wpscom_settings['screen_tablet'] ) && ! empty( $wpscom_settings['screen_tablet'] ) ) {
				$screen_sm = filter_var( $wpscom_settings['screen_tablet'], FILTER_SANITIZE_NUMBER_INT );
			}

			if ( isset( $wpscom_settings['screen_desktop'] ) && ! empty( $wpscom_settings['screen_desktop'] ) ) {
				$screen_md = filter_var( $wpscom_settings['screen_desktop'], FILTER_SANITIZE_NUMBER_INT );
			}

			if ( isset( $wpscom_settings['screen_large_desktop'] ) && ! empty( $wpscom_settings['screen_large_desktop'] ) ) {
				$screen_lg = filter_var( $wpscom_settings['screen_large_desktop'], FILTER_SANITIZE_NUMBER_INT );
			}

			if ( isset( $wpscom_settings['layout_gutter'] ) && ! empty( $wpscom_settings['layout_gutter'] ) ) {
				$gutter    = filter_var( $wpscom_settings['layout_gutter'], FILTER_SANITIZE_NUMBER_INT );
				$gutter    = ( $gutter < 2 ) ? 2 : $gutter;
			}

			if ( isset( $wpscom_settings['site_style'] ) && ! empty( $wpscom_settings['site_style'] ) ) {
			 ...
			 * BRANDING
			 */
			$brand_primary = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['color_brand_primary'] ) );
			$brand_success = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['color_brand_success'] ) );
			$brand_warning = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['color_brand_warning'] ) );
			$brand_danger  = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['color_brand_danger'] ) );
			$brand_info    = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['color_brand_info'] ) );

			$link_hover_color = ( Compound_Color::get_brightness( $brand_primary ) > 50 ) ? 'darken(@link-color, 15%)' : 'lighten(@link-color, 15%)';

			$brand_primary_brightness = Compound_Color::get_brightness( $brand_primary );
			$brand_success_brightness = Compound_Color::get_brightness( $brand_success );
			$brand_warning_brightness = Compound_Color::get_brightness( $brand_warning );
			$brand_danger_brightness  = Compound_Color::get_brightness( $brand_danger );
			$brand_info_brightness    = Compound_Color::get_brightness( $brand_info );
			...
			$input_border_focus = ( Compound_Color::get_brightness( $brand_primary ) < 195 ) ? 'lighten(@brand-primary, 10%)' : 'darken(@brand-primary, 10%)';
			$navbar_border      = ( Compound_Color::get_brightness( $brand_primary ) < 50 ) ? 'lighten(@navbar-default-bg, 6.5%)' : 'darken(@navbar-default-bg, 6.5%)';
			 ...
			 * JUMBOTRON
			 */
			$font_jumbotron         = wpscom_process_font( $wpscom_settings['font_jumbotron'] );
			$jumbotron_bg           = $wpscom_settings['jumbo_bg'];
			$jumbotron_bg           = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $jumbotron_bg['background-color'] ) );
			$jumbotron_text_color   = '#' . str_replace( '#', '', $font_jumbotron['color'] );

			if ( $wpscom_settings['font_jumbotron_heading_custom'] == 1 ) {
				$font_jumbotron_headers = wpscom_process_font( $wpscom_settings['font_jumbotron_headers'] );
				...
				$jumbotron_headers_text_color  = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $font_jumbotron_headers['color'] ) );
			...
			if ( isset( $wpscom_settings['padding_base'] ) && !empty( $wpscom_settings['padding_base'] ) ) {
				$padding_base  = intval( $wpscom_settings['padding_base'] );
			} else {
				$padding_base = 6;
			}

			if ( isset( $wpscom_settings['general_border_radius'] ) && ! empty( $wpscom_settings['general_border_radius'] ) ) {
				$border_radius = filter_var( $wpscom_settings['general_border_radius'], FILTER_SANITIZE_NUMBER_INT );
			 ...
			 * MENUS
			 */
			$font_brand        = wpscom_process_font( $wpscom_settings['font_brand'] );

			$font_navbar       = wpscom_process_font( $wpscom_settings['font_navbar'] );
			$navbar_bg         = '#' . str_replace( '#', '', Compound_Color::sanitize_hex( $wpscom_settings['navbar_bg'] ) );
			$navbar_height     = filter_var( $wpscom_settings['navbar_height'], FILTER_SANITIZE_NUMBER_INT );
			$navbar_text_color = '#' . str_replace( '#', '', $font_navbar['color'] );
			$brand_text_color  = '#' . str_replace( '#', '', $font_brand['color'] );
			$navbar_border     = ( Compound_Color::get_brightness( $navbar_bg ) < 50 ) ? 'lighten(@navbar-default-bg, 6.5%)' : 'darken(@navbar-default-bg, 6.5%)';
			$gfb = $wpscom_settings['grid_float_breakpoint'];

			if ( Compound_Color::get_brightness( $navbar_bg ) < 165 ) {
			...
			if ( isset( $wpscom_settings['navbar_margin_top'] ) && ! empty( $wpscom_settings['navbar_margin_top'] ) ) {
				$variables .= '@navbar-margin-top:       ' . $wpscom_settings['navbar_margin_top'] . 'px;';
			} else {
		...
		public function compiler() {
			...
			if ( isset( $wpscom_settings['minimize_css'] ) && $wpscom_settings['minimize_css'] == 1 ) {
				$compress = true;
			} else {
				$compress = false;
			}
			...
			try {
				...
				$parser = new Less_Parser( $options );
                                $parser->ModifyVars(array(
                                   'assets-url' =>  '"' . COMPOUND_ASSETS_URL . '"'
                                ));
				...
				// Enable gradients
				if ( $wpscom_settings['gradients_toggle'] == 1 ) {
				...
				if ( isset( $wpscom_settings['user_less'] ) && ! empty( $wpscom_settings['user_less'] ) ) {
					$parser->parse( $wpscom_settings['user_less'] );
				}
				...
				$parser->parse( apply_filters( 'wpscom_compiler', '' ) );
			...
			return apply_filters( 'wpscom_compiler_output', $css );
		}
		...
		public function include_wrapper() {
			...
			return $wpscom_layout->include_wrapper();
		}

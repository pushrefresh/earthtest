 ...
 */
function wpscom_customizer_fields() {
	...
	$settings = array(
		'colors_section' => array(
			...
			'fields' => array(
				'body_bg' => array(
					'label' => __( 'Background', 'wpscom' ),
					'type'  => 'background',
					'style' => 'body, .wrap.main-section .content .bg, .form-control, .btn, .panel',
					'priority' => 1,
				),
				'color_brand_primary' => array(
					'label' => __( 'Accent', 'wpscom' ),
					'type'  => 'color',
					'style' => 'a',
					'priority' => 3,
				),
				'navbar_bg' => array(
					'label' => __( 'NavBar', 'wpscom' ),
					'type'  => 'color',
					'style' => '',
					'priority' => 4,
				),
				// 'font_base' => array(
				// 	'label'    => __( 'Base Font', 'wpscom' ),
				// 	'type'     => 'font',
				// 	'style'    => '',
				// 	'priority' => 5,
				// ),
 ...
 */
function wpscom_customizer( $wp_customize ) {
	global $wpscom_settings;

	$sections = wpscom_customizer_fields();
			...
			if ( 'background' == $args['type'] || 'color' == $args['type'] ) {
				if ( 'background' == $args['type'] ) { // Background-color setting
					...
					$wp_customize->add_setting( $field,
						array(
							'default'    => $wpscom_settings[$field]['background-color'],
							'type'       => 'theme_mod',
				...
				$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $field, array(
					...
					// Add control
					$wp_customize->add_control( new Compound_Google_WebFont_Control( $wp_customize, $field, array(
				    'label'       => $args['label'],
				    'section'     => $section['slug'],
				    'settings'    => $field,
				    'priority'    => isset( $args['priority'] ) ? $args['priority'] : null,
...
add_action( 'customize_register', 'wpscom_customizer' );

/*
 * Applies the customizer styles to the preview screen.
 */
function wpscom_background_css() {
	$sections = wpscom_customizer_fields();
	...
	foreach ( $sections as $section ) {
		...
		foreach ( $fields as $field => $args ) {
			...
			if ( 'background' == $args['type'] ) {
				...
				if ( 'body_bg' == $field ) {
					$bg_brightness = Compound_Color::get_brightness( $value );
					// Set an "accent" color depending on the background's brightness
					if ( $bg_brightness > 120 ) {
						$accent = Compound_Color::adjust_brightness( $value, -20 );
						$border = Compound_Color::adjust_brightness( $value, -30 );
					} else {
						$accent = Compound_Color::adjust_brightness( $value, 20 );
						$border = Compound_Color::adjust_brightness( $value, 30 );
					}
					...
					if ( Compound_Color::lumosity_difference( $value, '#ffffff' ) > 5 ) {
						$text = '#ffffff';
					} elseif ( Compound_Color::lumosity_difference( $value, '#222222' ) > 5 ) {
						$text = '#222222';
					}
				...
				if ( 'color_brand_primary' == $field ) {
					...
					$brightness = Compound_Color::get_brightness( $value );
					if ( $brightness < 195 ) {
						$border = Compound_Color::adjust_brightness( $value, -20 );
						$text_c = '#fff';
					} else {
						$border = Compound_Color::adjust_brightness( $value, 20 );
						$text_c = '#333';
...
add_action( 'wp_head', 'wpscom_background_css', 210 );
 ...
 */
function wpscom_customizer_copy_options() {
	global $wpscom_settings;
	...
	foreach ( $sections as $section ) {
		...
		foreach ( $fields as $field => $args ) {
			...
			if ( 'background' == $args['type'] ) {
				...
				if ( 'body_bg' == $field ) {
					$wpscom_settings['html_bg']['background-color'] = $value;
					...
					$wpscom_settings['font_base']['color'] = $text;
					$wpscom_settings['font_h1']['color'] = $text;
					$wpscom_settings['font_h2']['color'] = $text;
					$wpscom_settings['font_h3']['color'] = $text;
					$wpscom_settings['font_h4']['color'] = $text;
					$wpscom_settings['font_h5']['color'] = $text;
					$wpscom_settings['font_h6']['color'] = $text;
				}

				// Copy the theme_mod to our settings array
				$wpscom_settings[$field]['background-color'] = $value;
				// Clean up theme mods
				remove_theme_mod( $field );

			} elseif ( 'font' == $args['type'] ) {
				// Copy the theme_mod to our settings array
				$wpscom_settings[$field]['font-family'] = $value;
				// Clean up theme mods
				remove_theme_mod( $field );
			} else {
				// Copy the theme_mod to our settings array
				$wpscom_settings[$field] = $value;
				// Clean up theme mods
				remove_theme_mod( $field );
			}
	...
	update_option( COMPOUND_OPT_NAME, $wpscom_settings );

	$compiler = new Compound_Less_PHP();
	add_action( 'customize_save_after', array( $compiler, 'makecss' ), 77 );

}
add_action( 'customize_save_after', 'wpscom_customizer_copy_options', 75 );
...
if ( class_exists( 'WP_Customize_Control' ) ) {
  class Compound_Google_WebFont_Control extends WP_Customize_Control {
    public $type = 'select';
    ...
    public function render_content() { ?>
        ...
        <label>
          ...
          <select <?php $this->link(); ?>>
            ...
            <?php
              $fonts = wpscom_google_webfonts_array();
 ...
 */
function wpscom_google_webfonts_array() {

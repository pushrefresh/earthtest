...
if ( !class_exists( 'Compound_Advanced' ) ) {

	/**
	* The "Advanced" module
	*/
	class Compound_Advanced {
		...
		function __construct() {
			...
			if ( isset( $wpscom_settings['jquery_cdn_toggler'] ) && $wpscom_settings['jquery_cdn_toggler'] == 1 ) {
				add_action( 'wp_enqueue_scripts', array( $this, 'jquery_cdn' ), 101 );
				add_action( 'wp_head',            array( $this, 'jquery_local_fallback' ) );
			}

			if ( isset( $wpscom_settings['nice_search'] ) && $wpscom_settings['nice_search'] == 1 ) {
			...
			if ( isset( $wpscom_settings['post_excerpt_length'] ) ) {
				define( 'POST_EXCERPT_LENGTH', $wpscom_settings['post_excerpt_length'] );
			}
		...
		*/
		...
		function user_css() {
			...
			if ( trim( $header_scripts ) != '' ) {
				wp_add_inline_style( 'wpscom_css', $header_scripts );
			}
		}
		...
		function scripts() {
			...
			if ( $settings['retina_toggle'] == 1 ) {
				wp_register_script( 'retinajs', COMPOUND_ASSETS_URL . '/js/vendor/retina.js', false, null, true );

...
if ( ! class_exists( 'Compound_Less_PHP' ) ) {

	/**
	* The Compound Compiler
	*/
	class Compound_Less_PHP {
		...
		function __construct() {
			...
			add_filter( 'wpscom_main_stylesheet_url', array( $this, 'stylesheet_url' ) );
			add_filter( 'wpscom_main_stylesheet_ver', array( $this, 'stylesheet_ver' ) );
			...
			add_action( 'redux/options/' . COMPOUND_OPT_NAME . '/compiler' , array( $this, 'makecss' ) );
		}
		...
		public static function file( $target = 'path', $echo = false ) {
			...
			if ( ! get_transient( 'wpscom_stylesheet_path' ) || ! get_transient( 'wpscom_stylesheet_uri' ) ) {
				...
				$file_name = '/wpscom-style' . $cssid . '.css';
				...
				if ( ! get_transient( 'wpscom_stylesheet_path' ) || ! get_transient( 'wpscom_stylesheet_uri' ) ) {
					set_transient( 'wpscom_stylesheet_path', $css_path, 24 * 60 *60 );
					set_transient( 'wpscom_stylesheet_uri', $css_uri, 24 * 60 *60 );
				}
			}

			$css_path = get_transient( 'wpscom_stylesheet_path' );
			$css_uri  = get_transient( 'wpscom_stylesheet_uri' );

			$value = ( $target == 'url' ) ? $css_uri : $css_path;

			// Get the file version based on its filemtime
			if ( $target == 'ver' ) {
				if ( ! get_transient( 'wpscom_stylesheet_time' ) ) {
					set_transient( 'wpscom_stylesheet_time', filemtime( $css_path ), 24 * 60 * 60 );
				}

				$value = get_transient( 'wpscom_stylesheet_time' );
			}
			...
			if ( $current_screen->parent_base == 'themes' ) {
				...
				if ( ! file_exists( $filename ) ) {
					if ( ! $wp_filesystem->put_contents( $filename, ' ', FS_CHMOD_FILE ) ) {
						$content = __( 'The following file does not exist and must be so in order to utilise this theme. Please create this file.', 'wpscom' );
						$content .= '<br>' . __( 'Try visiting the theme options and clicking the "Reset All" button to attempt automatically creating it.', 'wpscom' );
						$content .= '<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="' . $filename . '" target="_blank">' . $filename . '</a>';
						add_settings_error( 'wpscom', 'create_file', $content, 'error' );
						settings_errors();
					}
				} else {
					if ( ! is_writable( $filename ) ) {
						$content = __( 'The following file is not writable and must be so in order to utilise this theme. Please update the permissions.', 'wpscom' );
						$content .= '<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="' . $filename . '" target="_blank">' . $filename . '</a>';
		...
		public static function makecss() {
			global $wp_filesystem, $wpscom_framework;
			...
			$content .= $wpscom_framework->compiler();
			...
			if ( is_writeable( $file ) || ( ! file_exists( $file ) && is_writeable( dirname( $file ) ) ) ) {
				if ( ! $wp_filesystem->put_contents( $file, $content, FS_CHMOD_FILE ) ) {
					return apply_filters( 'wpscom_css_output', $content );
				}
			}
			// Force re-building the stylesheet version transient
			delete_transient( 'wpscom_stylesheet_time' );
			delete_transient( 'wpscom_stylesheet_path' );
			delete_transient( 'wpscom_stylesheet_uri' );
		}
	}
}
